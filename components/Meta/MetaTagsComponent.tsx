...
const MetaTagsComponent = ({ title, description, imageSrc, date }: props) => {
  ...
  const imageUrl = origin + (imageSrc ?? "/assets/ps-cover.png");

  const metaDescription = (
    description ??
    "Podcast solutions is a platform to host and manage podcasts. We offer audio transcription, AI content generation and much more."
  ).substring(0, 150);

  const myTitle = title ? `PS - ${title}` : "Podcast Solutions's Blog";
  ...
  return (
    <Head>
      ...
      <meta name="twitter:domain" content="https://blog.podcastsolutions.org" />
